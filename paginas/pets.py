...
def editar_pet_dialog():
    ...
    with st.form("editar_pet", clear_on_submit=False):
        ...
        with col1:
            ...
            peso_pet = st.number_input("Peso (em kg)", value=pet['peso'], min_value = 0.00, max_value=1000.00,
            step = 0.10)
        ...
        with col2:
            ...
            altura_pet = st.number_input("Altura (em cm)", value = pet['altura'], min_value=0, max_value=300, step=1)
...
with st.form("cadastro_pet", clear_on_submit=True):
    ...
    with col1:
        ...
        especie_pet = st.selectbox("Espécie *", options=[
            "Cachorro", "Gato", "Pássaro", "Coelho", "Hamster", "Peixe", "Réptil", "Outro"], index=None, placeholder="Selecione a espécie")
        raca_pet = st.text_input("Raça *", placeholder="Ex: Golden Retriever, SRD, Persa...")
        peso_pet = st.number_input("Peso (em kg)", value = 0.0, min_value = 0.00, max_value=1000.00,
        step = 0.10)
